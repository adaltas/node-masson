...
var EventEmitter, Run, Tree, context, crypto, each, flatten, merge, multimatch, pad, util, _ref;
  ...
  setImmediate((function(_this) {
    return function() {
      ...
      return each(config.servers).parallel(true).on('item', function(server, next) {
        ...
        return each(config.servers).parallel(true).on('item', function(server, next) {
          var ctx;
          if ((params.hosts != null) && multimatch(server.host, params.hosts).indexOf(server.host) === -1) {
            return next();
          }
          ...
          return _this.actions(server.host, params.command, params, function(err, actions) {
            ...
            return actionRun = each(actions).on('item', function(action, next) {
              var attempts, disregard_done, done, emit_action, retry, run, timeout;
              if (action.skip) {
                return next();
              }
              ctx.middleware = action;
              retry = action.retry != null ? actions.retry : 2;
              attempts = 0;
