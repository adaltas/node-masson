...
export default async (config) => {
  ...
  // Normalize argument
  if (!Array.isArray((config.masson.search))) {
    config.masson.search = [config.masson.search];
  }
  // Merge all configurations toguether
  return merge(
    config,
    ...(
      await each(config.masson.search, (search) => {
        // Filter discovery by file extensions
        return glob(config.masson.patterns, {
          cwd: search,
        }).then((files) =>
